...
export const applyGlobalSelect = () =>
  ({
    id: true,
    creationTime: true,
    creatorId: true,
  ...
  }) as FindOptionsSelect<BaseEntity>;
...
const handleMinMax = (minDate: Date, maxDate: Date) => {
  ...
  return And(
    MoreThanOrEqual(new Date(minDate)),
    LessThanOrEqual(new Date(maxDate))
  );
};
